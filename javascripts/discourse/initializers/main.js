...
export default {
  name: "discourse-theme-component",
  initialize() {
    withPluginApi("0.8.31", (api) => {
      const AGENDA_PATH = "/c/pabs-conference-2025/";
      const TOPICS_PATH = "/t/";
      ...
      api.onPageChange((newUrl) => {
        ...
        if (!lastPath.includes(AGENDA_PATH) && newUrl.includes(AGENDA_PATH)) {
          console.log(
            "[Agenda] Forcing full reload because user is ENTERING",
            AGENDA_PATH
          );
        ...
        if (
          lastPath.includes(AGENDA_PATH) &&
          !newUrl.includes(AGENDA_PATH) &&
          !newUrl.includes(TOPICS_PATH)
        ) {
          console.log(
            "[Agenda] Forcing full reload because user left",
            AGENDA_PATH
          );
          window.location = newUrl;
          return;
    ...
    function applyHighlighting(row, titleText) {
      const highlightRules = [
        {
          keyword: "Invite",
          cssClass: "invite-event",
          hideThumbnail: false,
          removeExcerpt: false,
        },
        {
          keyword: "Breakout Session",
          cssClass: "sessions-event",
          hideThumbnail: false,
          removeExcerpt: false,
        },
        {
      ...
      if (matchedRule) {
        ...
        if (matchedRule.removeExcerpt) {
          const excerptDiv = row.querySelector(
            "div[style*='font-size: 14px; color: rgb(102, 102, 102); margin-top: 4px;']"
          );
          if (excerptDiv) {
            excerptDiv.remove();
          }
    ...
    function isInsidePABSConference() {
      const categoryDropdown = document.querySelector(
        ".select-kit-selected-name .badge-category__name"
      );
      return (
        categoryDropdown &&
        categoryDropdown.textContent.trim() === "PABS Conference 2025"
      );
    }
    ...
    function runAfterEmberRender(callback, attempts = 0) {
      ...
      requestAnimationFrame(() => {
        if (document.querySelector("tbody.topic-list-body")) {
          ...
          logWarning(
            `⚠️ Waiting for Ember rendering... (${attempts + 1}/${MAX_RETRIES})`
          );
          setTimeout(
            () => runAfterEmberRender(callback, attempts + 1),
            CHECK_INTERVAL
          );
        }
      });
    ...
    function processAgendaEvents() {
      ...
      if (processingInProgress && now - lastRunTime < cooldownTime) {
        logWarning(
          "⏳ Process already running or recently executed. Skipping."
        );
        return;
      }
      ...
      runAfterEmberRender(() => {
        if (!isInsidePABSConference()) {
          logWarning(
            "⚠️ Not inside PABS conference. Continuing without processing..."
          );
        ...
        if (!topicList) {
          retryCount++;
          logWarning(
            `⚠️ No topic list found, retrying... (${retryCount}/${MAX_RETRIES})`
          );
        ...
        function parseEventDate(dateText) {
          ...
          if (dateMatch) {
            ...
            let parsedDate = new Date(
              Date.UTC(currentYear, month - 1, day, 5, 0, 0)
            );
        ...
        function getEventDate(row) {
          ...
          return eventDateElement
            ? parseEventDate(eventDateElement.textContent.trim())
            : null;
        }
        ...
        nonPinnedPosts.forEach((row) => {
          ...
          const eventTitle = row.querySelector(
        ...
        document
          .querySelectorAll(".topic-list-item-event .date")
          .forEach((el) => {
            if (
              el.nextElementSibling &&
              el.nextElementSibling.classList.contains("time-only-display")
            ) {
